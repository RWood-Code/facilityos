...
export default function Reports() {
  ...
  return (
    <div>
      <PageHeader title="Reports & Analytics" subtitle="Compliance, maintenance, and operational reporting"
        actions={
      ...
      <TabBar tabs={[{value:'compliance',label:'Pool Compliance'},{value:'tests',label:'Test Log'},{value:'maintenance',label:'Maintenance'},{value:'custom',label:'Custom Reports'}]} active={tab} onChange={setTab} />

      {loading ? <Spinner /> : (<>
        {tab==='compliance' && (
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="px-5 py-3.5 border-b border-gray-100"><h3 className="text-sm font-semibold text-gray-900">Compliance by Pool — Last {days} Days</h3></div>
            {compliance.length===0 ? <div className="py-12 text-center text-gray-400 text-sm">No test data for this period</div> : (
              <table className="w-full">
                <thead><tr className="border-b border-gray-100 text-xs text-gray-500 uppercase tracking-wider">
                  <th className="text-left px-5 py-3">Pool</th><th className="text-right px-5 py-3">Tests</th><th className="text-right px-5 py-3">Compliant</th><th className="text-right px-5 py-3">Non-Compliant</th><th className="text-right px-5 py-3">Rate</th><th className="text-left px-5 py-3">Last Tested</th>
                </tr></thead>
                  ...
                  {compliance.map(r => {
                    ...
                    return (
                      <tr key={r.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-5 py-3 text-right"><span className={"font-semibold text-sm "+(rate>=95?'text-emerald-600':rate>=80?'text-amber-600':'text-red-600')}>{rate!=null?`${rate}%`:'—'}</span></td>
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="border-b border-gray-100 text-xs text-gray-500 uppercase tracking-wider">
                  <th className="text-left px-4 py-2">Date</th><th className="text-left px-4 py-2">Pool</th><th className="text-right px-4 py-2">FAC</th><th className="text-right px-4 py-2">pH</th><th className="text-right px-4 py-2">Temp</th><th className="text-left px-4 py-2">By</th><th className="text-left px-4 py-2">Result</th>
                </tr></thead>
                <tbody className="divide-y divide-gray-50">
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 max-w-xl">
            ...
            <div className="flex gap-2 mb-4">
              ...
              <Btn size="sm" onClick={async () => {
                ...
                setNewReportName('');
                dbQuery('saved_reports:list').then(setSavedReports);
                toast('Report saved');
            ...
            <ul className="space-y-2">
              {savedReports.map(r => (
                <li key={r.id} className="flex justify-between items-center py-2 border-b border-gray-50 text-sm">
                  ...
                  <Btn variant="ghost" size="sm" onClick={async () => { await dbQuery('saved_reports:delete', r.id); dbQuery('saved_reports:list').then(setSavedReports); }}>Delete</Btn>
                </li>
              ))}
            </ul>
          </div>
        ...
        {tab==='maintenance' && (
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="px-5 py-3.5 border-b border-gray-100"><h3 className="text-sm font-semibold text-gray-900">Work Order Summary</h3></div>
            <div className="grid grid-cols-5 divide-x divide-gray-100 py-6 text-center">
              {[['Open',workSummary.open||0,'text-blue-600'],['In Progress',workSummary.in_progress||0,'text-amber-600'],['On Hold',workSummary.on_hold||0,'text-gray-500'],['Completed',workSummary.completed||0,'text-emerald-600'],['Urgent',workSummary.urgent||0,'text-red-600']].map(([l,v,c])=>(
                <div key={l}><div className={"text-3xl font-bold "+c}>{v}</div><div className="text-xs text-gray-400 mt-1">{l}</div></div>
              ))}
            </div>
          </div>
        )}
